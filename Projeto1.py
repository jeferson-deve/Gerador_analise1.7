...
def calculo_solucao(nome, arquivar_op=0):
    ...
    if arquivar_op == 1:
        ...
        print('Calculo solido %s' % nome, file=file_tes)
        for i in range(quantidade):
            ...
            print('%.4f' % placa1_a, '\t', '%.4f' % placa1_b, file=file_tes)
            print('%.3f' % amostra_a, '\t\t', '%.3f' % amostra_b, file=file_tes)
            print('%.4f' % placa3a, '\t', '%.4f' % placa3b, file=file_tes)
    ...
    for i in range(quantidade):
        ...
        print('%.4f' % placa1_a, '\t', '%.4f' % placa1_b)
        print('%.3f' % amostra_a, '\t\t', '%.3f' % amostra_b)
        print('%.4f' % placa3a, '\t', '%.4f' % placa3b)
...
def calculo_nitrato(nome, arquivar_op=0):
    print('Calculo de concentração %s' % nome)
    ...
    if arquivar_op == 1:
        file_tes = open(file_name, 'a+')
        print('Calculo de concentração %s' % nome, file=file_tes)
...
def calculo_acido(nome, arquivar=0):
    print('Calculo concentraçao de Acido %s' % nome)
    quantidade = int(input('Quantos calculos gostaria: '))

    if arquivar == 1:
        file_tes = open(file_name, 'a+')
        print('Calculo concentraçao de Acido %s' % nome, file=file_tes)
        for i in range(quantidade):
